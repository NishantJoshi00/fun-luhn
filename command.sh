  ...
  "test")
    ...
    case "$1" in
      "-l" | "--language")
        ...
        { time \
          while read -r input output; do 
            ...
            ((total_cases++))
               ...
               <(
                cat \
                  <(cat ../../testcases/good | awk '{ print $1 " Ok" }') \
                  <(cat ../../testcases/bad | awk '{print $1 " Failed"}') \
                  | shuf
               )
        } 2>&1
        ...
        success_rate=$(echo "scale=2; ($success_cases / $total_cases) * 100" | bc)
        failure_rate=$(echo "scale=2; ($failure_cases / $total_cases) * 100" | bc)
        error_rate=$(echo "scale=2; ($error_cases / $total_cases) * 100" | bc)
